...
export function rewriteIndexHtml(config, html, publicOrigin) {
  const escapedEnginePath = config.enginePath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const localEnginePattern = new RegExp(
    `https?:\\/\\/(?:127\\.0\\.0\\.1|localhost):\\d+${escapedEnginePath}`,
    "g",
  );
  ...
  nextHtml = nextHtml.replace(localEnginePattern, engineUrl);
